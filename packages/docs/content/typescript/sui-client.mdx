...
The OneChain TypeScript SDK provides a `SuiClient` class to connect to a network's JSON-RPC server. Use
`SuiClient` for all JSON-RPC operations.

## Connecting to a OneChain network

To establish a connection to a network, import `SuiClient` from `@onelabs/sui/client` and pass the
