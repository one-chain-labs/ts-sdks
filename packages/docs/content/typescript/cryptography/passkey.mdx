...
Similar to other Keypair classes, the OneChain TypeScript SDK provides the `PasskeyKeypair` that handles
...
To use, import the `PasskeyKeypair` class from `@onelabs/sui/keypairs/passkey`.
...
import {
	...
	PasskeyKeypair,
} from '@onelabs/sui/keypairs/passkey';
```
