...
The majority of changes in this release are in the `@onelabs/sui` package, but the changes to API
...
## Changes to `@onelabs/sui`

As part of the 1.0 release the `@onelabs/sui.js` package has been renamed to `@onelabs/sui`

To upgrade to the new version, uninstall the old package and install the new one:

```sh npm2yarn
npm uninstall @onelabs/sui.js
...
```diff
- import { SuiClient } from '@onelabs/sui.js'
+ import { SuiClient } from '@onelabs/sui'
```

### `@onelabs/sui/transactions`
...
```diff
- import { Transactions } from '@onelabs/sui/transactions'
+ import { Commands } from '@onelabs/sui/transactions'
...
The `publicKeyFromSuiBytes` has been moved to `@onelabs/sui/verify`

```diff
- import { publicKeyFromSuiBytes } from '@onelabs/sui/multisig'
+ import { publicKeyFromSuiBytes } from '@onelabs/sui/verify'
```
...
We previously deprecated the `generic` helper exported from `@onelabs/bcs`. This helper has now been
entirely removed.
...
```diff
- import { getSentTransactionBlocksWithLinks, ZkSendLinkBuilder } from '@onelabs/zksend'
+ import { getSentTransactionsWithLinks, ZkSendLinkBuilder } from '@onelabs/zksend'
...
```ts
import { signAndExecuteTransaction, signTransaction } from '@onelabs/wallet-standard';
