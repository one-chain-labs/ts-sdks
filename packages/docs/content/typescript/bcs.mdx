...
The `@onelabs/sui/bcs` package extends `@onelabs/bcs` with OneChain specific scheme definitions.

To learn more about using BCS see the [BCS documentation](/bcs).

the `bcs` export of `@onelabs/sui/bcs` contains all the same exports as `bcs` from `@onelabs/bcs` plus
