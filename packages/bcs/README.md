...
To install, add the [`@onelabs/bcs`](https://www.npmjs.com/package/@onelabs/bcs) package to your
project:
...
The `@onelabs/bcs` library can be used to define schemas that can serialize and deserialize BCS
...
```ts
// Example: Generics
import { bcs, BcsType } from '@onelabs/bcs';
...
```ts
import { bcs, toHex } from '@onelabs/bcs';
...
transforms can also handle more complex types. For instance, `@onelabs/sui` uses the following
...
```ts
import { bcs, fromBase58, fromBase64, fromHex } from '@onelabs/bcs';
...
```ts
import { bcs, type InferBcsType, type InferBcsInput } from '@onelabs/bcs';
